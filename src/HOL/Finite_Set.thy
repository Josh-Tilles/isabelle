...
using `finite F` proof (induct rule: finite_induct) (* is there a way to clean this pattern up? *)
  case empty
  show "finite (h -` {} \<inter> A)" by simp

  case (insert x F)
  show "finite (h -` (insert x F) \<inter> A)"
  proof (subst vimage_insert)
    show "finite ((h -` {x} \<union> h -` F) \<inter> A)"
      using `inj_on h A` and insert.hyps
      by (simp add: finite_subset [OF inj_on_vimage_singleton] Int_Un_distrib2)
  qed
qed
